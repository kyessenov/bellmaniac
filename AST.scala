...
trait Environment extends Logger {
  ...
  // Must be called after "offsetify"
  def inlineAll(p: List[Algorithm]) = {
    ...
    println("calls: " + calls.map(_.v))

    // do not call themselves
    val candidates = calls.filter {
      case a => Vars(a.expr) intersect (calls.map(_.v).toSet) isEmpty        
    }.toSet
    ...
    for (a @ Algorithm(v, args, pre, expr) <- p;
      ...
      yield Algorithm(v, args, pre, inline(expr))    
  }

  def compile(main: Algorithm, out: PrintStream = Console.out, 
      printer: PythonPrinter = Python) {
    //showGraph()     
    val all = inputs ::: main :: inlineAll(offsettify(refineAll))
      ...
      def push(path: Pred, e: Expr): Expr = 
        visit(e)(path, exprTransformer {
          case (path, App(u, uargs)) if u == c0.v =>      
            restricts.find {
              case (a, c1) => 
                println(path implies c1.pre.s(c1.args zip uargs))
